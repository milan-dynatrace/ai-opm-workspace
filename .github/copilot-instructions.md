...
Tasks are tracked as individual files in `Tasks/` with a generated dashboard in `Dashboard/Tasks.md`.

- **Creating tasks**: Use `/task` skill or ask in chat. Any skill that produces action items (meetings, decisions) will offer to create tasks.
- **Completing tasks**: Check the checkbox in `Dashboard/Tasks.md` in Obsidian. Next task-aware skill run syncs the change.
- **Reviewing tasks**: Use `/task-review` for ad-hoc triage and prioritization.
- **Daily planning**: `/today` reads active tasks and suggests focus items.
- Always include owner and deadline where known in the task file.
...
| **Monthly** | Reflection, progress review | Custom prompt (build your own) |
...
**Solution**: `/meeting` skill creates structured notes with decisions, action items as `- [ ]` checkboxes, and links to relevant context.
